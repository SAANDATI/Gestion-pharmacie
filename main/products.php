...
<a  href="index.php"><button class="btn btn-default btn-large" style="float: left;"><i class="icon icon-circle-arrow-left icon-large"></i> Back</button></a>
			...
			<?php 
			include('../connect.php');
				$result = $db->prepare("SELECT * FROM products where qty < 10 ORDER BY id_produit DESC");
				$result->execute();
...
<table class="hoverTable" id="resultTable" data-responsive="table" style="text-align: left;">
	...
	<tbody>
			...
			function formatMoney($number, $fractional=false) {
				...
				include('../connect.php');
				$result = $db->prepare("SELECT *, prix * qty as total FROM products ORDER BY id_produit DESC");
				$result->execute();
			...
			<td><?php echo $row['nom_de_famille']; ?></td>
			<td><?php echo $row['nom_du_produit']; ?></td>
			<td><?php echo $row['fournisseur']; ?></td>
			<td><?php echo $row['date_arrival']; ?></td>
			<td><?php echo $row['expiry_date']; ?></td>
			<td><?php $pprice=$row['prix']; echo formatMoney($pprice, true); ?></td>
			<td><?php echo $row['qty_sold']; ?></td>
			<td><?php echo $row['qty']; ?></td>
			<td><?php $total=$row['total']; echo formatMoney($total, true);?></td>			
			<td><a rel="facebox" title="Click to edit the product" href="editproduct.php?id=<?php echo $row['id_produit']; ?>"><button class="btn btn-warning"><i class="icon-edit"></i> </button> </a>
			<a href="#" id="<?php echo $row['id_produit']; ?>" class="delbutton" title="Click to Delete the product"><button class="btn btn-danger"><i class="icon-trash"></i></button></a></td>
			</tr>
			<?php
				}
			?>
